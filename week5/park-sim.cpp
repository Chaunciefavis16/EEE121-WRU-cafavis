...
private:
    ...
    void quickSort(vector<CarEntry> &entries, int low, int high)
    {
        if (low < high)
        {
            int pivot = partition(entries, low, high);
            cout << "PVT- " << entries[pivot].year << " "
                 << "PARTITION: ";
            ...
            {
                cout << "PVT-" << entries[pivot].year << " : " << parkingDatabase[k].plate << ", " << put_time(localtime(&parkingDatabase[k].entryTime), "%T") << ", "
                     << "NONE, "
                     << parkingDatabase[k].totalHours << ", "
                     << "P" << parkingDatabase[k].totalCost << ", " << put_time(localtime(&parkingDatabase[k].entryTime), "%F") << ", "
    ...
    int partition(vector<CarEntry> &entries, int low, int high)
    {
        CarEntry pivot = entries[high];
        int i = low - 1;
        for (int j = low; j <= high - 1; j++)
        {
            if (entries[j].year < pivot.year)
            {
                i++;
    ...
    void displayDatabase(const vector<CarEntry> &entries)
    {
...
public:
        ...
        for (int i = 0; i < parkingDatabase.size(); i++)
        {
            if (parkingDatabase[i].plate == plate)
            {
                ...
                cout << carOut.plate << "," << put_time(localtime(&carOut.entryTime), "%T") << ","
                     ...
                     << carOut.totalHours << ","
                     << "P" << carOut.totalCost << "," << put_time(localtime(&carOut.entryTime), "%F") << ","
    ...
    void bubbleSort()
    {
        for (int i = 0; i < parkingDatabase.size() - 1; i++)
        {
            for (size_t k = 0; k < parkingDatabase.size(); k++)
            {
                cout << "ITER-" << i << " : " << parkingDatabase[k].plate << ", " << put_time(localtime(&parkingDatabase[k].entryTime), "%T") << ", "
                     << "NONE, "
                     << parkingDatabase[k].totalHours << ", "
                     << "P" << parkingDatabase[k].totalCost << ", " << put_time(localtime(&parkingDatabase[k].entryTime), "%F") << ", "
                     << parkingDatabase[k].brand << ", " << parkingDatabase[k].model << ", " << parkingDatabase[k].year << endl;
    ...
    void quickSortEntries()
    {
        cout << "UNSORTED LIST: \n";
        displayDatabase(parkingDatabase);
        cout << endl;
        quickSort(parkingDatabase, 0, parkingDatabase.size() - 1);
    }
    ...
    void binarySearch(const string &plate)
    {
        ...
        while (left <= right)
        {
            ...
            cout << "MID-" << middle << " : " << parkingDatabase[middle].plate << ", " << put_time(localtime(&parkingDatabase[middle].entryTime), "%T") << ", "
                 << "NONE, "
                 << parkingDatabase[middle].totalHours << ", "
            ...
            {
                cout << endl
                     << parkingDatabase[middle].plate << ", " << put_time(localtime(&parkingDatabase[middle].entryTime), "%T") << ", "
